<template>
  <div class="add-task-form" role="form" aria-label="Add task form" tabindex="0">
    ...
    <div
      ...
      class="form-errors"
      ref="errors"
      aria-label="Errors from form submission"
      tabindex="0"
    >
    ...
    <div class="form row">
      ...
      <div class="group col-sm-6">
        ...
        <select id="task-importance" aria-required="true" v-model="newTask.importance">
    ...
    <div class="action">
      <button type="button" @click="addTask" aria-label="Add new task">Add Task</button>
      <button type="button" v-b-modal.discard-modal aria-label="Discard new task">Cancel</button>
    </div>

    <b-modal id="discard-modal" centered>
      <template v-slot:modal-header="{ close }">
        <h2>Discard task?</h2>
        <b-button @click="close()" aria-label="Close discard task model and return to form">x</b-button>
      </template>
      ...
      <template v-slot:modal-footer="{ cancel, discard }">
        <button type="button" @click="discardTask">Discard task</button>
        <button
          type="button"
